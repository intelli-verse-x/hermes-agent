...
def test_dispatcher_threads_delivery_mode_to_backend():
    ...
    Fork governance blocks destructive computer_use fail-closed when no
    interactive approver exists, so wire an approving callback for the
    duration of the dispatch (each invocation prompts fresh; nothing
    persists).
    """
...
# ---------------------------------------------------------------------------
# Phase C — approval freshness (fork governance: nothing persists)
#
# Upstream caches approve_session / always_approve unlocks per
# (action, delivery_mode, session) — see NousResearch/hermes-agent#67052.
# This fork deliberately persists NOTHING: every destructive invocation
# prompts fresh, which subsumes upstream's "background approval must not
# authorize foreground" property.
# ---------------------------------------------------------------------------
...
def test_background_approval_does_not_authorize_foreground():
    ...
    try:
        # Background click, approved.
        assert cu._request_approval("click", {}, "sess-A") is None
        # A second background click prompts again — approvals never persist.
        assert cu._request_approval("click", {}, "sess-A") is None
        assert len(seen) == 2
        # Foreground click on the SAME action also prompts fresh.
...
def test_approval_state_is_session_scoped():
    ...
    try:
        ...
        # Run B must prompt independently (nothing persists anywhere).
        n_before = len(calls)
...
def test_always_approve_does_not_persist():
    from tools.computer_use import tool as cu
    ...
    cu.set_approval_callback(cb)
    try:
        # always_approve authorizes only the invocation it was shown for.
        assert cu._request_approval("click", {}, "run-C") is None
        # The next mutation — foreground or not — prompts again.
        assert cu._request_approval("click", {"delivery_mode": "foreground"}, "run-C") is None
        assert len(calls) == 2
    finally:
        cu.set_approval_callback(None)
