...
import { audiobookLibraryPath, audiobookPositionPayload, debateScorePayload } from './engines/service-contracts'
...
const DEFAULT_MISSIONS: LapMission[] = [
  {
    claimed: false,
    description: 'Complete one generated quiz',
    id: 'daily-quiz',
    progress: 0,
    required: 1,
    rewardXp: 30,
  ...
  {
    claimed: false,
    description: 'Study five flashcards',
    id: 'daily-cards',
    progress: 0,
    required: 5,
    rewardXp: 25,
  ...
  {
    claimed: false,
    description: 'Win a realtime battle',
    id: 'weekly-battle',
    progress: 0,
    required: 1,
    rewardXp: 100,
...
function readMissions(): LapMission[] {
  try {
    ...
    if (stored.day === new Date().toISOString().slice(0, 10) && Array.isArray(stored.missions)) {
...
function mapQuestion(raw: Record<string, unknown>, index: number): LapQuestion | null {
  ...
  const rawOptions = answerRows.length
    ...
    : Array.isArray(raw.options ?? raw.choices)
      ? ((raw.options ?? raw.choices) as unknown[]).map(option =>
          typeof option === 'object' ? object(option) : { text: String(option) }
        )
      : []
...
function mapNote(raw: Record<string, unknown>): LapNote {
  ...
  const questions = rawQuestions
    .map((item, index) => mapQuestion(object(item), index))
    .filter((item): item is LapQuestion => Boolean(item))
  ...
  return {
    ...
    questions,
    status:
      status.includes('process') || status === 'queued' || status === 'pending'
        ? 'processing'
...
function Notice({ error, text }: { error?: boolean; text: string }) {
  return text ? (
    <p
      className={`mt-3 rounded p-2 text-xs ${error ? 'bg-red-950/50 text-red-100' : 'bg-black/20'}`}
...
async function loadNote(noteId: string): Promise<LapNote> {
  ...
  if (!note.id) {
    throw new Error('The note response did not contain a note ID.')
  }
...
function Library({ onSelect }: { onSelect: (noteId: string) => void }) {
  ...
  const load = () => {
    ...
    void productRequest<unknown>({ path: '/notes/recent?limit=50' })
      ...
      .catch(() =>
        productRequest<unknown>({ path: '/notes?limit=50' })
          .then(({ data }) => setNotes(rows(data, 'notes').map(mapNote)))
  ...
  return (
    <div className="grid gap-3">
      ...
      {filtered.map(note => (
        <button
          className="qv-glass-tile rounded-xl p-4 text-left"
          key={note.id}
          onClick={() => {
            onSelect(note.id)
        ...
        >
          ...
          <span className="mt-1 block text-xs text-muted-foreground">
            {note.type} · {note.status} · {note.questions.length} questions · {note.flashcards.length} cards
          </span>
        </button>
      ))}
      {!notes.length && !error && (
        <p className="text-sm text-muted-foreground">
          No notes yet. Create one from a URL, file, audio, image, or text.
        </p>
      )}
    </div>
  )
...
function CreateNote({ onCreated }: { onCreated: (noteId: string) => void }) {
  ...
  const chooseFile = async () => {
    const paths = await window.hermesDesktop.selectPaths({
      filters: [
        {
          extensions: ['pdf', 'doc', 'docx', 'png', 'jpg', 'jpeg', 'webp', 'mp3', 'm4a', 'wav'],
          name: 'Learning sources'
        }
      ],
      title: 'Choose a learning source'
    })
    const path = paths[0]
    ...
    setFile({
      ...
      mime: metadata.match(/^data:([^;]+)/)?.[1] ?? 'application/octet-stream'
    })
  }
  ...
  const poll = async (jobId: string) => {
    for (
      let attempt = 0;
      attempt < 48 && !cancelled.current && !abortController.current?.signal.aborted;
      attempt += 1
    ) {
      const { data } = await productRequest<Record<string, unknown>>({
        path: `/notes/jobs/${encodeURIComponent(jobId)}/status`
      })
      ...
      if (status === 'failed' || status === 'error') {
        throw new Error(String(data.error ?? 'Note processing failed.'))
      }
      await new Promise(resolve => window.setTimeout(resolve, Math.min(6_000, 2_000 + attempt * 250)))
    }

    if (!cancelled.current) {
      throw new Error('Note processing timed out. Retry from the library.')
    }
  }
  ...
  const create = async () => {
    ...
    try {
      ...
      if (noteId) {
        ...
        throw new Error('The create response contained neither noteId nor jobId.')
      }
    } catch (error) {
  ...
  return (
    <Card title="Create Link & Play note">
      <div className="mt-3 flex flex-wrap gap-2">
        {(['website', 'youtube', 'text', 'pdf', 'doc', 'audio', 'image'] as SourceType[]).map(type => (
          <Button
            key={type}
            onClick={() => {
              setSourceType(type)
      ...
      />
      {requiresFile ? (
        <Button className="mt-3" onClick={() => void chooseFile()} variant="outline">
          {file ? file.filename : 'Choose file'}
        </Button>
      ) : sourceType === 'text' ? (
        <textarea
          aria-label="Source text"
          className="mt-3 min-h-36 w-full rounded border p-3 text-sm"
      ...
      {progress > 0 && (
        <progress aria-label="Note processing progress" className="mt-3 w-full" max={100} value={progress} />
      )}
      <div className="mt-3 flex gap-2">
        <Button disabled={requiresFile ? !file : !source.trim()} onClick={() => void create()}>
          Create and generate
        </Button>
        <Button disabled={progress === 0} onClick={() => void cancel()} variant="outline">
          Cancel processing
        </Button>
      </div>
...
function NoteDetail({ noteId }: { noteId: string }) {
  ...
  if (!noteId) {
    return <Notice error text="Choose a note from the library first." />
  }

  return (
    <Card title={note?.title ?? 'Note detail'}>
      <p className="mt-2 text-xs text-muted-foreground">
        {note?.type ?? 'source'} · {note?.status ?? 'loading'}
      </p>
      {note?.text && <p className="mt-3 max-h-40 overflow-auto whitespace-pre-wrap text-sm">{note.text}</p>}
      <div className="mt-3 flex flex-wrap gap-2">
        {['quiz', 'flashcards', 'chat', 'debate', 'mind-map', 'speed-read', 'audiobook', 'figurine', 'explainer'].map(
          route => (
            <Button key={route} onClick={() => openNativeSurface('link-play', route)} size="sm" variant="outline">
              {route}
            </Button>
          )
        )}
      ...
      <Button
        ...
        onClick={() =>
          void productRequest({
            body: { generateAudiobook: false, generateFlashcards: true, generateQuiz: true },
            method: 'POST',
            path: `/notes/${encodeURIComponent(noteId)}/generate-all/async`
          })
            .then(() => setError('Generation job queued. Refresh shortly.'))
            .catch(reason => setError(String(reason)))
        }
      >
...
function Quiz({ noteId }: { noteId: string }) {
  ...
  useEffect(() => {
    if (noteId) {
      void loadNote(noteId)
        .then(note => setQuestions(note.questions))
    ...
    return (
      <Card title="Quiz result">
        ...
        <Button
          className="mt-3"
          onClick={() =>
            void playRpc('quizverse_lap_submit_progress', {
              activity: 'quiz',
              ...
              .then(() => setMessage('Progress synced.'))
              .catch(() => setMessage('Result saved locally; reward sync is unavailable.'))
          }
        >
          Save result
        </Button>
  ...
  return (
    <Card title="Generated quiz">
      {question ? (
        <>
          <p className="mt-2 text-xs">
            Question {answers.length + 1}/{questions.length}
          </p>
          <p className="mt-3 font-semibold">{question.text}</p>
          <div className="mt-3 grid gap-2">
            {question.options.map(option => (
              <Button key={option.id} onClick={() => setAnswers(current => [...current, option.id])} variant="outline">
                {option.text}
              </Button>
            ))}
          </div>
        ...
        <p className="mt-2 text-sm">Loading generated questions…</p>
      )}
...
function Flashcards({ noteId }: { noteId: string }) {
  ...
  useEffect(() => {
    if (noteId) {
      void loadNote(noteId).then(note => setCards(note.flashcards))
    }
  }, [noteId])
  ...
  return (
    <Card title="Flashcards">
      {card ? (
        <>
          <button
            className="mt-3 min-h-40 w-full rounded-xl border p-5 text-center"
            onClick={() => setBack(value => !value)}
            type="button"
          >
            <b>{back ? card.back : card.front}</b>
            <span className="mt-2 block text-xs">Tap to flip</span>
          </button>
          <div className="mt-3 flex gap-2">
            <Button
              disabled={index === 0}
        ...
        <p className="mt-2 text-sm">No generated flashcards are available yet.</p>
      )}
    </Card>
  )
}
...
function Chat({ noteId }: { noteId: string }) {
  ...
  useEffect(() => {
    ...
    void productRequest<unknown>({ path: `/notes/${encodeURIComponent(noteId)}/with-chat` })
      .then(({ data }) => {
    ...
    try {
      ...
      if (!activeChat) {
        const { data } = await productRequest<unknown>({
          body: { title: 'Desktop Study Chat' },
          method: 'POST',
          path: `/notes/${encodeURIComponent(noteId)}/chat`
        })
      ...
      if (!activeChat) {
        throw new Error('The chat service did not return a chat ID.')
      }
      ...
      const accumulate = createSseAccumulator(event =>
        append(
          typeof event === 'string'
            ? event
            : String(object(event).content ?? object(event).delta ?? object(event).text ?? '')
        )
      )
      await productStream(
        {
  ...
  return (
    <Card title="Grounded note chat">
      <div aria-live="polite" className="mt-3 grid max-h-72 gap-2 overflow-auto">
        {messages.map((message, index) => (
          <p
            className={`rounded p-3 text-sm ${message.role === 'user' ? 'ml-8 bg-violet-800/40' : 'mr-8 bg-black/20'}`}
      ...
      >
        ...
        {streaming && (
          <Button onClick={cancelStream} type="button" variant="outline">
            Stop
          </Button>
        )}
      </form>
...
function SpeedReader({ noteId }: { noteId: string }) {
    ...
    void productRequest<unknown>({
      ...
      .then(({ data }) => {
        ...
        const tokens = Array.isArray(root.tokens)
          ? root.tokens.map(token => String(object(token).word ?? object(token).w ?? ''))
          : []
        setText(tokens.join(' '))
      })
      .catch(reason => {
  ...
  return (
    <Card title="Speed reading">
      <p aria-live="polite" className="mt-6 min-h-20 text-center text-3xl font-bold">
        {words[index] ?? (words.length ? 'Complete' : 'Loading…')}
      </p>
...
function MindMap({ noteId }: { noteId: string }) {
  ...
  return (
    <Card title="Note mind map">
      <Button className="mt-3" disabled={!noteId} onClick={generate}>
        Generate map
      </Button>
      {nodes.length > 0 && (
        <svg aria-label="Generated mind map" className="mt-3 w-full rounded border bg-black/20" viewBox="0 0 480 320">
          {nodes.map((node, index) => {
            const angle = (index / nodes.length) * Math.PI * 2
            const x = index === 0 ? centerX : centerX + Math.cos(angle) * 150
            const y = index === 0 ? centerY : centerY + Math.sin(angle) * 110

            return (
              <g key={String(node.id ?? index)}>
                {index > 0 && (
                  <line stroke="currentColor" strokeOpacity=".35" x1={centerX} x2={x} y1={centerY} y2={y} />
                )}
                <circle cx={x} cy={y} fill={index === 0 ? '#7c3aed' : '#1e293b'} r={index === 0 ? 34 : 27} />
                <text fill="white" fontSize="10" textAnchor="middle" x={x} y={y + 3}>
                  {String(node.label ?? node.title ?? node.text ?? '').slice(0, 18)}
                </text>
              </g>
            )
          })}
...
function RealtimeProduct({ noteId, mode }: { mode: 'arena' | 'battle'; noteId: string }) {
  ...
  useEffect(() => {
    ...
    void playRpc('quizverse_lap_get_leaderboard', { scope })
      .then(value => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.hermesDesktop.quizverse?.onPlayRealtimeEvent?.(event => {
      if (event.id !== battle?.connectionId || event.type !== 'match-data') {
        return
      }
      const payload = object(event.data)
      setBattle(current =>
        current
          ? { ...current, opponentScore: Number(payload.opponent_score ?? payload.score ?? current.opponentScore) }
          : current
      )
    })
    ...
    return () => {
      ...
      if (battle?.connectionId) {
        void window.hermesDesktop.quizverse?.playRealtimeClose(battle.connectionId)
      }
    }
  }, [battle?.connectionId, mode])

  const claimMission = (mission: LapMission) => {
    if (mission.claimed || mission.progress < mission.required) {
      return
    }
  ...
  const findBattle = () =>
    void playRpc('quizverse_lap_battle_find', { note_id: noteId })
      .then(async value => {
        ...
        if (!data.match_id || !questions.length) {
          throw new Error('No playable battle match is available.')
        }
        const connection = await window.hermesDesktop.quizverse?.playRealtimeConnect()

        if (!connection) {
          throw new Error('Realtime battle transport is unavailable.')
        }
        ...
        setBattle({
          ...
          opponent: String(data.opponent_name ?? 'Opponent'),
          opponentScore: 0,
          questions
        })
        setAnswers([])
  ...
  if (mode === 'arena') {
    ...
    return (
      <Card title="Knowledge Arena">
        <div className="mt-3 flex gap-2">
          {(['missions', 'leaderboard', 'badges'] as const).map(value => (
            <Button
              key={value}
              onClick={() => setArenaView(value)}
              size="sm"
              variant={arenaView === value ? 'default' : 'outline'}
            >
              {value}
            </Button>
          ))}
        ...
        {arenaView === 'missions' && (
          <div className="mt-3 grid gap-2">
            {missions.map(mission => (
              <div className="rounded border p-3" key={mission.id}>
                <b className="text-sm">{mission.title}</b>
                <p className="text-xs">
                  {mission.description} · {mission.progress}/{mission.required}
                </p>
                <progress className="mt-2 w-full" max={mission.required} value={mission.progress} />
                <Button
                  className="mt-2"
                  disabled={mission.claimed || mission.progress < mission.required}
                  onClick={() => claimMission(mission)}
                  size="sm"
                >
                  {mission.claimed ? 'Claimed' : `Claim ${mission.rewardXp} XP`}
                </Button>
              </div>
            ))}
          </div>
        ...
        {arenaView === 'leaderboard' && (
          <>
            <div className="mt-3 flex gap-2">
              {['global', 'friends', 'regional'].map(value => (
                <Button
                  key={value}
            ...
            <ol className="mt-3 grid gap-1">
              {leaderboard.map((entry, index) => (
                <li className="text-xs" key={String(entry.userId ?? entry.owner_id ?? index)}>
                  #{String(entry.rank ?? index + 1)} {String(entry.displayName ?? entry.username ?? 'Player')} ·{' '}
                  {String(entry.xp ?? entry.score ?? 0)} XP
                </li>
              ))}
            </ol>
          </>
        ...
        {arenaView === 'badges' && (
          <div className="mt-3 grid grid-cols-2 gap-2">
            {badges.length ? (
              badges.map(mission => (
                <div className="rounded border p-4 text-center" key={mission.id}>
                  🏅<span className="mt-1 block text-xs">{mission.title}</span>
                </div>
              ))
            ) : (
              <p className="text-sm">Complete and claim missions to unlock badges.</p>
            )}
          </div>
        )}
        <Notice error text={status} />
      </Card>
    )
  }
  ...
  const score = battle
    ? answers.reduce((sum, answer, index) => sum + (answer === battle.questions[index]?.correctOptionId ? 1 : 0), 0)
    : 0
  ...
  const answerQuestion = (optionId: string) => {
    ...
    setAnswers(next)
    void window.hermesDesktop.quizverse?.playRealtimeSend(battle.connectionId, 2, {
      answer_id: optionId,
    ...
    if (next.length === battle.questions.length) {
      const finalScore = next.reduce(
        (sum, answer, index) => sum + (answer === battle.questions[index]?.correctOptionId ? 1 : 0),
        0
      )
      void window.hermesDesktop.quizverse?.playRealtimeSend(battle.connectionId, 3, {
        match_id: battle.matchId,
        score: finalScore
      })
      void playRpc('quizverse_lap_submit_progress', {
        activity: 'battle_result',
        match_id: battle.matchId,
        noteId,
        score: finalScore,
  ...
  return (
    <Card title="Note Battle">
      {!battle && (
        <>
          <p className="mt-2 text-sm">Find a native Nakama battle using this note’s generated questions.</p>
          <Button className="mt-3" disabled={!noteId} onClick={findBattle}>
            Find match
          </Button>
        </>
      )}
      {battle && question && (
        <>
          <p className="mt-2 text-xs">
            Against {battle.opponent} ({battle.opponentScore}) · {answers.length + 1}/{battle.questions.length}
          </p>
          <p className="mt-3 font-semibold">{question.text}</p>
          <div className="mt-3 grid gap-2">
            {question.options.map(option => (
              <Button key={option.id} onClick={() => answerQuestion(option.id)} variant="outline">
                {option.text}
              </Button>
            ))}
          </div>
      ...
      {battle && !question && (
        <>
          <p className="mt-2 text-2xl font-bold">
            {score}/{battle.questions.length} · opponent {battle.opponentScore}
          </p>
          <p className="mt-1 text-xs">Battle result submitted to the realtime match and progress service.</p>
          <Button
            className="mt-3"
            onClick={() => {
              void window.hermesDesktop.quizverse?.playRealtimeClose(battle.connectionId)
...
function Debate({ noteId }: { noteId: string }) {
  ...
  useEffect(() => {
    ...
    void productRequest<unknown>({ path: `/notes/${encodeURIComponent(noteId)}/debate-topics` })
      .then(({ data }) => {
  ...
  const start = () => {
    ...
    if (mode === 'timed') {
      Object.assign(body, { timeLimitSeconds: 120, totalTimeLimitSeconds: 600 })
    }

    if (mode === 'rapid-fire') {
      Object.assign(body, { maxArgumentLength: 280, timeLimitSeconds: 30 })
    }
    ...
    if (mode === 'oxford') {
      body.includesCrossExamination = true
    }
    void productRequest<unknown>({
      body,
      method: 'POST',
  ...
  const submitArgument = async () => {
    ...
    const append = (value: string) =>
      setTranscript(current =>
        current.map((entry, index) => (index === current.length - 1 ? { ...entry, text: entry.text + value } : entry))
      )
    const accumulate = createSseAccumulator(event =>
      append(
        typeof event === 'string'
    ...
    try {
      await productStream(
        {
          path: `/notes/chat/${encodeURIComponent(chatId)}/stream?message=${encodeURIComponent(text)}`,
  ...
  return (
    <Card title="Debate">
      <div className="mt-3 flex flex-wrap gap-2">
        {(['start', 'timed', 'rapid-fire', 'multi-round', 'oxford'] as const).map(value => (
          <Button key={value} onClick={() => setMode(value)} size="sm" variant={mode === value ? 'default' : 'outline'}>
            {value}
          </Button>
        ))}
      </div>
      ...
      />
      <div className="mt-3 flex gap-2">
        <Button onClick={() => setPosition('for')} variant={position === 'for' ? 'default' : 'outline'}>
          Argue for
        </Button>
        <Button onClick={() => setPosition('against')} variant={position === 'against' ? 'default' : 'outline'}>
          Argue against
        </Button>
      </div>
      {!session && (
        <Button className="mt-3" disabled={!noteId || !topic.trim()} onClick={start}>
          Start debate
        </Button>
      )}
      {session && (
        <>
          <p className="mt-3 text-xs">
            Round {String(session.currentRound ?? 1)}/{String(session.totalRounds ?? (mode === 'oxford' ? 5 : 1))} ·
            phase {String(session.phase ?? 'argument')}
          </p>
          <div aria-live="polite" className="mt-3 grid max-h-64 gap-2 overflow-auto">
            {transcript.map((entry, index) => (
              <p
                className={`rounded p-3 text-sm ${entry.role === 'user' ? 'ml-8 bg-violet-800/40' : 'mr-8 bg-black/20'}`}
          ...
          <textarea
            aria-label="Debate argument"
            className="mt-3 min-h-24 w-full rounded border p-3"
            onChange={event => setArgument(event.target.value)}
            value={argument}
          />
          <div className="mt-3 flex gap-2">
            <Button disabled={!argument.trim()} onClick={() => void submitArgument()}>
              Submit argument
            </Button>
            <Button onClick={() => void advance()} variant="outline">
              {mode === 'oxford' ? 'Advance Oxford phase' : 'Next round'}
            </Button>
            <Button onClick={() => void scoreDebate()} variant="outline">
              Score debate
            </Button>
          </div>
        </>
      ...
      {score && (
        <div className="mt-3 rounded border p-4">
          <p className="text-xl font-bold">
            You {String(score.userScore ?? score.user_score ?? 0)} · AI {String(score.aiScore ?? score.ai_score ?? 0)}
          </p>
          <p className="mt-2 text-sm">{String(score.feedback ?? '')}</p>
          <p className="mt-2 text-xs">
            Logic {String(object(score.breakdown).logic ?? '—')} · Evidence{' '}
            {String(object(score.breakdown).evidence ?? '—')} · Persuasion{' '}
            {String(object(score.breakdown).persuasion ?? '—')} · Clarity{' '}
            {String(object(score.breakdown).clarity ?? '—')}
          </p>
        </div>
      )}
      <Notice error text={message} />
...
function GeneratedJob({
  ...
  const start = async () => {
    try {
      ...
      for (let attempt = 0; attempt < 60; attempt += 1) {
        ...
        if (['completed', 'ready', 'failed', 'error'].includes(phase)) {
          return
        }
  ...
  return (
    <Card title={title}>
      ...
      {Boolean(activeJobId.current) && (
        <Button className="ml-2 mt-3" onClick={() => void cancel()} variant="outline">
          Cancel job
        </Button>
      )}
      {result && (
        <div className="mt-3 rounded border p-3">
          <p className="text-sm font-semibold">{String(result.title ?? result.status ?? 'Generation result')}</p>
          {artifactUrl &&
            (isVideo ? (
              <video className="mt-3 max-h-80 w-full" controls src={artifactUrl} />
            ) : (
              <img alt={`${title} result`} className="mt-3 max-h-80 w-full object-contain" src={artifactUrl} />
            ))}
          {Boolean(result.description) && <p className="mt-2 text-xs">{String(result.description)}</p>}
        </div>
      )}
      <Notice error text={message} />
    </Card>
...
function Figurine({ noteId }: { noteId: string }) {
  ...
  const loadPreview = () =>
    void productRequest<unknown>({ path: `/notes/${encodeURIComponent(noteId)}/figurine-prompt` })
      .then(({ data }) => {
  ...
  return (
    <div className="grid gap-3">
      <Card title="Figurine preview">
        <Button disabled={!noteId} onClick={loadPreview} variant="outline">
          Preview styles and prompt
        </Button>
        {preview && (
          <div className="mt-3 rounded border p-4">
            <p className="font-semibold">{String(preview.title ?? preview.subject ?? 'Figurine concept')}</p>
            <p className="mt-2 text-xs">{String(preview.prompt ?? preview.description ?? '')}</p>
          </div>
        )}
        <div className="mt-3 flex flex-wrap gap-2">
          {['cartoon', 'realistic', 'chibi', 'collectible'].map(value => (
            <Button
              key={value}
      ...
      {preview && (
        <GeneratedJob
          body={{ style, ...(subject.trim() ? { subject: subject.trim() } : {}) }}
          path={`/notes/${encodeURIComponent(noteId)}/generate-figurine`}
          statusPath={jobId => `/figurine/job/${encodeURIComponent(jobId)}/status`}
...
function Audiobook({ noteId }: { noteId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    await productRequest<unknown>({ path: audiobookLibraryPath(session.userId) })
      .then(({ data }) => {
  ...
  const savePosition = (seconds: number) => {
    ...
    if (!bookId || whole === lastSavedSecond.current || whole % 10 !== 0) {
      return
    }
    lastSavedSecond.current = whole
    void productRequest({
      body: audiobookPositionPayload(seconds),
      method: 'POST',
      path: `/audiobook/${encodeURIComponent(bookId)}/position`
    }).catch(() => undefined)
  }

  return (
    <div className="grid gap-3">
      <Card title="Audiobook library">
        ...
        <div className="mt-3 grid gap-2">
          {books.map(book => (
            <button
              ...
              key={String(book.id ?? book.audiobook_id)}
              onClick={() => setSelected(book)}
              type="button"
            >
              <b>{String(book.title ?? 'Study audiobook')}</b>
              <span className="block text-xs">
                {String(book.status ?? 'ready')} · {String(book.duration_seconds ?? '—')} seconds
              </span>
            </button>
          ))}
        </div>
          ...
          <div className="mt-4">
            ...
            <ol className="mt-3 grid gap-1">
              {chapters.map((chapter, index) => (
                <li key={String(chapter.id ?? index)}>
                  <button
                    className="w-full rounded p-2 text-left text-xs hover:bg-black/10"
                    onClick={() => {
                      if (audio.current) {
                        audio.current.currentTime = Number(chapter.start_seconds ?? chapter.start ?? 0)
                        void audio.current.play()
                      }
                    }}
                    type="button"
                  >
                    {index + 1}. {String(chapter.title ?? `Chapter ${index + 1}`)} ·{' '}
                    {String(chapter.start_seconds ?? chapter.start ?? 0)}s
                  </button>
                </li>
              ))}
            </ol>
      ...
      {userId ? (
        <GeneratedJob
          body={{
            audiobookType: 'StudyAudio',
            includeChapters: true,
            language: 'en',
            narrationStyle: 'Educational',
            noteId,
            sourceType: 'ExistingNote',
            userId
          }}
          path="/audiobook/create"
          statusPath={jobId => `/audiobook/status/${encodeURIComponent(jobId)}`}
          title="Generate audiobook"
        />
      ) : (
        <Card title="Generate audiobook">
          <p className="mt-2 text-sm">Connecting your QuizVerse identity…</p>
        </Card>
      )}
    </div>
  )
...
function SrsReview() {
  ...
  const load = () =>
    void productRequest<unknown>({ path: '/flashcards/srs/queue' })
      .then(({ data }) => {
        const root = unwrap(data)
  ...
  return (
    <Card title="Spaced repetition queue">
      {card ? (
        <>
          <p className="mt-3 text-lg font-semibold">{String(card.front ?? card.question ?? '')}</p>
          {revealed && <p className="mt-3 rounded bg-black/10 p-3">{String(card.back ?? card.answer ?? '')}</p>}
          {!revealed ? (
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {['Again', 'Hard', 'Good', 'Easy'].map((label, rating) => (
                <Button key={label} onClick={() => rate(rating + 1)} size="sm" variant="outline">
                  {label}
                </Button>
              ))}
            </div>
        ...
        <>
          <p className="mt-3 text-sm">No more cards are due.</p>
          <Button className="mt-3" onClick={load} variant="outline">
            Refresh queue
          </Button>
        </>
      )}
...
export function LinkPlaySurface({ route }: LinkPlaySurfaceProps) {
  ...
  if (route === 'library') {
    return <Library onSelect={setNoteId} />
  }

  if (route === 'create') {
    return <CreateNote onCreated={setNoteId} />
  }
  ...
  if (route === 'quiz') {
    return <Quiz noteId={noteId} />
  }

  if (route === 'flashcards') {
    return <Flashcards noteId={noteId} />
  }

  if (route === 'chat') {
    return <Chat noteId={noteId} />
  }

  if (route === 'arena' || route === 'battle') {
    return <RealtimeProduct mode={route} noteId={noteId} />
  }

  if (route === 'speed-read') {
    return <SpeedReader noteId={noteId} />
  }
  ...
  if (route === 'debate') {
    return <Debate noteId={noteId} />
  }

  if (route === 'mind-map') {
    return <MindMap noteId={noteId} />
  }

  if (route === 'audiobook') {
    return <Audiobook noteId={noteId} />
  }

  if (route === 'figurine') {
    return <Figurine noteId={noteId} />
  }

  if (route === 'explainer') {
    return (
      <GeneratedJob
        body={{ duration_seconds: 20, topic: 'Explain the key ideas' }}
        path={`/notes/${encodeURIComponent(noteId)}/generate-explainer-video`}
        statusPath={jobId => `/notes/${encodeURIComponent(noteId)}/explainer-status/${encodeURIComponent(jobId)}`}
