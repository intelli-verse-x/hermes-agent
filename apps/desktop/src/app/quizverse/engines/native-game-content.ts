...
export const SPELL_PUZZLES: readonly SpellPuzzle[] = [
  { id: 'gen-spell-01', center: 'T', letters: ['T', 'A', 'I', 'L', 'C', 'P', 'O'], words: ['TACO', 'TAIL', 'PACT', 'LAIT', 'PATIO', 'PILOT', 'LOOT', 'TOOT', 'TOPIC', 'OPTIC', 'CAPITOL', 'COITAL'], pangram: 'CAPITOL' },
  { id: 'gen-spell-02', center: 'R', letters: ['R', 'E', 'A', 'D', 'G', 'N', 'I'], words: ['DARE', 'DEAR', 'RAIN', 'RIDE', 'RAID', 'GRADE', 'GRAIN', 'GRIND', 'RANGE', 'READING', 'DRAINER', 'GARDENING'], pangram: 'GARDENING' },
  { id: 'gen-spell-03', center: 'O', letters: ['O', 'C', 'M', 'P', 'S', 'E', 'T'], words: ['COME', 'COPE', 'COST', 'POEM', 'POET', 'SCOPE', 'COMET', 'SMOTE', 'TEMPO', 'COMPOSE', 'COMPOST', 'COMPETE', 'COMPOTES'], pangram: 'COMPOTES' },
  { id: 'gen-spell-04', center: 'N', letters: ['N', 'I', 'G', 'H', 'T', 'L', 'E'], words: ['NINE', 'NIGHT', 'HINGE', 'LINEN', 'TENT', 'THINE', 'LIGHTEN', 'LENGTHEN', 'ENLIGHTEN', 'LINING', 'TILING'], pangram: 'ENLIGHTEN' }
]

export const CROSSWORD_PUZZLES: readonly CrosswordPuzzle[] = [
  {
    id: 'gen-cw-01', theme: 'Rules', difficulty: 'easy',
    grid: [['T', 'A', 'B', 'O', 'O'], ['.', '.', 'A', '.', '.'], ['B', 'A', 'S', 'I', 'S'], ['.', '.', 'I', '.', '.'], ['F', 'A', 'C', 'E', 'T']],
    clues: {
      across: [
  ...
  {
    id: 'gen-cw-02', theme: 'Music & Clocks', difficulty: 'easy',
    grid: [['W', 'A', 'T', 'C', 'H'], ['.', '.', 'E', '.', '.'], ['L', 'E', 'M', 'O', 'N'], ['.', '.', 'P', '.', '.'], ['P', 'R', 'O', 'O', 'F']],
    clues: {
      across: [
...
export function wordShareGrid(target: string, guesses: readonly string[]): string {
  ...
  return guesses.map(guess => scoreWord(target, guess).map(score => icons[score]).join('')).join('\n')
}
...
export function spellScore(words: readonly string[], puzzle: SpellPuzzle): number {
  return words.reduce((score, word) => score + (word.length === 4 ? 1 : word.length) + (isPangram(word, puzzle) ? 7 : 0), 0)
}
...
export function validateSpellWord(word: string, puzzle: SpellPuzzle): string | null {
  ...
  if (normalized.length < 4) {return 'Words must contain at least four letters.'}

  if (!normalized.includes(puzzle.center)) {return `Words must use ${puzzle.center}.`}

  if ([...normalized].some(letter => !puzzle.letters.includes(letter))) {return 'That word uses a letter outside the hive.'}

  if (!puzzle.words.includes(normalized) && normalized !== puzzle.pangram) {return 'That word is not in today’s source list.'}

  return null
}

export function imposterIndices(players: number, count: number, seed: number): number[] {
  return seededShuffle(Array.from({ length: players }, (_, index) => index), seed).slice(0, count).sort((a, b) => a - b)
}
...
export const VOYAGE_THEMES: readonly VoyageTheme[] = [
  theme('space', ['🪐', '🚀', '🌕', '☄️', '🛸', '⭐', '🌌', '🌠'], ['SATURN', 'SUN', 'STAR', 'MARS'], ['MARS', 'MOON', 'STAR', 'COMET', 'ORBIT', 'SUN'], [{ emoji: '🪐', label: 'Saturn' }, { emoji: '🚀', label: 'Rocket' }, { emoji: '🌕', label: 'Moon' }, { emoji: '☄️', label: 'Comet' }], [
    ...
    ['Who was the first person to walk on the Moon?', ['Buzz Aldrin', 'Yuri Gagarin', 'Neil Armstrong', 'John Glenn'], 2],
    ...
    ['Which is the closest star to Earth (besides the Sun)?', ['Sirius', 'Alpha Centauri', 'Proxima Centauri', 'Vega'], 2],
  ...
  theme('animals', ['🐶', '🐱', '🦁', '🐘', '🦒', '🐧', '🦋', '🐠'], ['PANTHER', 'HARE', 'RAT', 'HEN'], ['LION', 'BEAR', 'WOLF', 'TIGER', 'SHARK', 'BEE'], [{ emoji: '🦁', label: 'Lion' }, { emoji: '🐘', label: 'Elephant' }, { emoji: '🦒', label: 'Giraffe' }, { emoji: '🐧', label: 'Penguin' }], [
    ...
    ['Which animal is a primate that uses sign language in famous studies?', ['Dolphin', 'Gorilla', 'Parrot', 'Dog'], 1],
  ...
  theme('world', ['🌍', '🗽', '🗼', '🏔️', '🏝️', '🕌', '🌋', '🏯'], ['ISLAND', 'LAND', 'SAND', 'LANE'], ['NILE', 'PARIS', 'TOKYO', 'OCEAN', 'RIVER', 'ASIA'], [{ emoji: '🗼', label: 'Paris' }, { emoji: '🗽', label: 'New York' }, { emoji: '🏔️', label: 'Everest' }, { emoji: '🌋', label: 'Volcano' }], [
    ...
    ['The Great Barrier Reef is off the coast of which country?', ['Indonesia', 'Philippines', 'Australia', 'Fiji'], 2],
  ...
  theme('science', ['🧪', '⚗️', '🔬', '🧬', '⚛️', '💡', '🧫', '🌡️'], ['REACTOR', 'REACT', 'CORE', 'TRACE'], ['ATOM', 'CELL', 'GENE', 'LIGHT', 'WATER', 'HEAT'], [{ emoji: '🧪', label: 'Lab' }, { emoji: '🧬', label: 'DNA' }, { emoji: '⚛️', label: 'Atom' }, { emoji: '🌡️', label: 'Thermometer' }], [
    ...
    ['DNA stands for what?', ['Deoxyribonucleic acid', 'Dynamic nucleic action', 'Diribose nuclear acid', 'Direct nucleotide assembly'], 0],
  ...
  theme('history', ['🏛️', '📜', '⚔️', '🏺', '🗿', '🏰', '👑', '🛡️'], ['EMPIRE', 'PRIME', 'RIPE', 'PIER'], ['ROME', 'EGYPT', 'KING', 'EMPIRE', 'PYRAMID', 'WAR'], [{ emoji: '🏛️', label: 'Greek Temple' }, { emoji: '🗿', label: 'Easter Island' }, { emoji: '🏰', label: 'Castle' }, { emoji: '👑', label: 'Crown' }], [
  ...
  theme('tech', ['💻', '📱', '🤖', '🛰️', '💾', '🖥️', '🔌', '📡'], ['POINTER', 'POINT', 'PRINT', 'PORT'], ['CODE', 'BYTE', 'PIXEL', 'CLOUD', 'APP', 'WEB'], [{ emoji: '💻', label: 'Laptop' }, { emoji: '🤖', label: 'Robot' }, { emoji: '🛰️', label: 'Satellite' }, { emoji: '📱', label: 'Phone' }], [
    ['Who founded Microsoft?', ['Steve Jobs', 'Bill Gates', 'Elon Musk', 'Mark Zuckerberg'], 1],
    ['What does CPU stand for?', ['Computer Personal Unit', 'Central Processing Unit', 'Core Power Utility', 'Common Program Use'], 1],
    ['Which company makes the iPhone?', ['Samsung', 'Sony', 'Apple', 'LG'], 2],
    ['HTML stands for what?', ['Hyper Text Markup Language', 'High Tech Modern Language', 'Home Tool Modular Library', 'Hyperlink Tagging Markup'], 0],
    ...
    ['Bitcoin’s creator goes by what pseudonym?', ['Vitalik Buterin', 'Satoshi Nakamoto', 'John McAfee', 'Hal Finney'], 1],
    ['GPU stands for what?', ['General Purpose Unit', 'Graphics Processing Unit', 'Game Power Utility', 'Graphical Pixel User'], 1],
    ['Which programming language is most associated with web browsers?', ['Python', 'Java', 'JavaScript', 'Ruby'], 2],
    ['The cloud is essentially what?', ['Local storage', 'Someone else’s computer', 'A type of network cable', 'Mobile data only'], 1]
  ])
]
...
export function buildWordSearch(words: readonly string[], seed: number, size = 10): {
  grid: string[][]
  ...
  for (const rawWord of words) {
    ...
    for (let attempt = 0; attempt < 200; attempt += 1) {
      ...
      if (cells.some(([row, column], index) =>
        row < 0 || column < 0 || row >= size || column >= size ||
        (grid[row]![column] !== '' && grid[row]![column] !== word[index])
      )) {continue}

      cells.forEach(([row, column], index) => { grid[row]![column] = word[index]! })
