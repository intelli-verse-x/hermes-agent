...
describe('QuizVerse repository service contracts', () => {
  ...
  it('normalizes the published bracket metadata without inventing rounds', () => {
    expect(normalizeTournamentBracket({
      data: {
        bracket_id: 'br_1',
        exists: true,
    ...
    expect(() => normalizeTournamentBracket({
      data: { bracket_id: 'br_1', exists: true, public_dashboard_url: 'https://evil.example/br_1', round: 1, total_rounds: 2 }
    })).toThrow(/untrusted/)
  })
  ...
  it('uses the certificate claim id for the certificate lookup envelope', () => {
    ...
    expect(normalizeTournamentCertificate({
