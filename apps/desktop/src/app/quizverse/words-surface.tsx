...
function Provenance({ content }: { content: WordsDailyEnvelope }) {
  return (
    <div className="text-xs text-muted-foreground">
      <p>
        {content.server_decided
          ? 'Server-authoritative daily seed'
          : `Deterministic offline seed · ${content.degraded ?? 'local'}`}
        {' · '}UTC {content.utc_day}
      </p>
      <p>
      ...
      {content.content_provenance && (
        <p>
          Source: {content.content_provenance} · License: {content.content_license}
        </p>
      )}
      {content.content_error && (
        <p className="mt-2 rounded border border-amber-500/40 bg-amber-950/30 p-2 text-amber-100" role="status">
          Full Words banks are not configured or failed integrity validation: {content.content_error}. Using approved
          minimal original General fallback content
          {content.skin === 'gre-easy' ? '; this is not the full GRE bank' : ''}.
        </p>
      )}
    </div>
  )
...
function DailyWord({ content }: { content: WordsDailyEnvelope }) {
  const distributed = bankItems(content.puzzle_bank)
    .map(String)
  ...
  const submit = async () => {
    ...
    if (nextPhase === 'won') {
      ...
      setMessage(`Guess ${nextGuesses.length} submitted.`)
    }
  }
    ...
    <GameCard title="Daily Word">
      ...
      <div className="mt-3 flex max-w-md gap-2">
        <input
          ...
          value={guess}
        />
        <Button disabled={guess.length !== 5 || phase !== 'playing'} onClick={() => void submit()} size="sm">
          Guess
        </Button>
      </div>
...
function GroupsGame({ content }: { content: WordsDailyEnvelope }) {
  const distributed = bankItems(content.puzzle_bank).filter(
    (candidate): candidate is readonly WordGroup[] =>
      ...
      candidate.every(
        group =>
          group &&
          typeof group === 'object' &&
          Array.isArray((group as WordGroup).words) &&
          (group as WordGroup).words.length === 4
      )
  )
  ...
  const check = () => {
    const match = puzzle.find(
      group => group.words.length === selected.length && group.words.every(word => selected.includes(word))
    )
    ...
    if (match) {
      ...
      if (nextSolved.length === 4) {
        completeStreakDay({ kind: 'words', mode: 'groups', skin: content.skin })
      }
  ...
  return (
    <GameCard title="Groups">
          ...
          return (
            <button
              ...
              className={cn(
                ...
                done ? 'bg-emerald-800' : selected.includes(word) ? 'bg-violet-700' : 'bg-black/20'
              )}
              disabled={done || ended}
              key={word}
              onClick={() =>
      ...
      <Button className="mt-3" disabled={selected.length !== 4 || ended} onClick={check} size="sm">
        Submit group
      </Button>
      <Status message={message} />
      {(ended || solved.length > 0) && (
        <div className="mt-3 grid gap-2">
          {puzzle
            .filter(group => ended || solved.includes(group.title))
            .map(group => (
      ...
      {ended && (
        <CopyResult
          text={`QuizVerse Groups ${content.day_index}\n${attempts.map(ok => (ok ? '🟩🟩🟩🟩' : '🟥🟥🟥🟥')).join('\n')}`}
        />
      )}
      <Button
        className="mt-3"
        onClick={() => {
          ...
          setSolved([])
          setMessage('')
          persist({ attempts: [], mistakes: 0, solved: [] })
        }}
        size="sm"
        variant="outline"
      >
...
function SpellGame({ content }: { content: WordsDailyEnvelope }) {
  ...
  const submit = async () => {
    ...
    if (error && !error.includes('source list')) {
      setMessage(error)
    ...
    <GameCard title="Spell">
      ...
      <div className="mt-3 flex gap-2">
        <input
          ...
          value={typed}
        />
        <Button onClick={() => setTyped(value => value.slice(0, -1))} variant="outline">
          Delete
        </Button>
        <Button disabled={!typed} onClick={() => void submit()}>
          Enter
        </Button>
      </div>
      <p className="mt-3 text-sm font-semibold">
        {found.length} found · {spellScore(found, puzzle)} points
      </p>
      <div className="mt-2 flex flex-wrap gap-1">
        {found.map(word => (
          <span className="rounded bg-violet-900/50 px-2 py-1 text-xs" key={word}>
            {word}
          </span>
        ))}
      </div>
...
function CrosswordGame({ content }: { content: WordsDailyEnvelope }) {
  const distributed = bankItems(content.puzzle_bank).filter((candidate): candidate is CrosswordPuzzle => {
    ...
    return (
      ...
      puzzle.grid.every(row => Array.isArray(row) && row.length === 5) &&
      Boolean(puzzle.clues?.across && puzzle.clues?.down)
    )
  })
  ...
  const update = (row: number, column: number, value: string) => {
    ...
    persist({
      entries: next,
      revealed,
      solved: puzzle.grid.every((line, r) => line.every((letter, c) => letter === '.' || next[r]?.[c] === letter))
    })
  }
  ...
  return (
    <GameCard title={`Crossword · ${puzzle.theme}`}>
          ...
          row.map((letter, c) =>
            letter === '.' ? (
              ...
              <input
                ...
                className={cn(
                  'aspect-square min-w-0 rounded border text-center font-bold uppercase',
                  checked && entries[r]?.[c] && entries[r]?.[c] !== letter && 'border-red-500 bg-red-950'
                )}
                key={`${r}:${c}`}
                maxLength={1}
      ...
      <div className="mt-3 flex gap-2">
        <Button onClick={() => setChecked(value => !value)} size="sm" variant="outline">
          {checked ? 'Hide check' : 'Check'}
        </Button>
        <Button
          onClick={() => {
            setEntries(empty)
            persist({ entries: empty, revealed, solved: false })
          }}
          size="sm"
          variant="outline"
        >
        ...
        <Button
          disabled={!selectedCell || !revealConfirmed}
          onClick={() => {
            if (!selectedCell) {
              return
            }
            ...
            persist({
              entries: next,
              revealed: true,
              solved: puzzle.grid.every((line, r) =>
                line.every((letter, c) => letter === '.' || next[r]?.[c] === letter)
              )
            })
      ...
      <label className="mt-2 flex gap-2 text-xs">
        <input checked={revealConfirmed} onChange={event => setRevealConfirmed(event.target.checked)} type="checkbox" />
        Reveal costs this puzzle’s clean solve.
      </label>
      <Status
        message={
          solved
            ? revealed
              ? 'Crossword solved with a reveal. Clean-solve streak and reward were not granted.'
              : 'Crossword solved. Streak saved.'
            : checked
              ? 'Incorrect cells are marked in red.'
              : ''
        }
      />
      {solved && (
        <CopyResult
          text={`QuizVerse Crossword ${content.day_index}\n${puzzle.grid.map(row => row.map(cell => (cell === '.' ? '⬛' : '⬜')).join('')).join('\n')}`}
        />
      )}
    </GameCard>
  )
...
function ImposterGame({ content }: { content: WordsDailyEnvelope }) {
  ...
  const imposters = useMemo(
    () => imposterIndices(players, imposterCount, content.seed),
    [content.seed, imposterCount, players]
  )
  ...
  useEffect(() => {
    ...
    if (phase === 'result') {
      completeStreakDay({ kind: 'words', mode: 'imposter', skin: content.skin })
    }
    ...
    return (
      <GameCard title="Imposter">
        ...
        <p className="mt-2 text-xs">
          Pass-and-play. Every player privately reveals a word; the imposter receives a close decoy.
        </p>
        <label className="mt-3 block text-xs">
          ...
          <input
            ...
            onChange={event => {
              ...
              setPlayers(count)
              setImposterCount(current => Math.min(current, Math.max(1, count - 2)))
            }}
            type="range"
            value={players}
          />
        ...
        <label className="mt-3 flex gap-2 text-xs">
          <input checked={decoy} onChange={event => setDecoy(event.target.checked)} type="checkbox" />
          Give imposters a close decoy word
        </label>
        <Button className="mt-3" onClick={() => setPhase('reveal')}>
          Start round
        </Button>
      </GameCard>
    )
      ...
      <GameCard title={`Pass to Player ${cursor + 1}`}>
        {!shown ? (
          ...
          <>
            ...
            >
              {cursor + 1 === players ? 'Begin discussion' : `Hide and pass to Player ${cursor + 2}`}
            </Button>
          </>
        )}
      </GameCard>
  ...
  if (phase === 'discuss') {
    return (
      <GameCard title="Discuss">
        <p className="text-sm">
          Give one clue each without saying the word. Category: <b>{puzzle.category}</b>
        </p>
        <Button className="mt-3" onClick={() => setPhase('vote')}>
          Start private vote
        </Button>
      </GameCard>
    )
  ...
  if (phase === 'vote') {
    return (
      <GameCard title={`Player ${cursor + 1} votes`}>
        <div className="grid grid-cols-2 gap-2">
          {Array.from({ length: players }, (_, index) => index)
            ...
            .map(index => (
              <Button
                key={index}
                onClick={() => setVotes(current => ({ ...current, [cursor]: index }))}
                variant={votes[cursor] === index ? 'default' : 'outline'}
              >
                Player {index + 1}
              </Button>
            ))}
        ...
        <Button
          className="mt-3"
          disabled={votes[cursor] === undefined}
          onClick={() => {
            if (cursor + 1 === players) {
              setPhase('result')
            } else {
        ...
        >
          {cursor + 1 === players ? 'Show result' : `Lock vote and pass to Player ${cursor + 2}`}
        </Button>
      </GameCard>
    )
  }
  ...
  return (
    <GameCard title={result?.imposterWin ? 'Imposter wins' : 'Table wins'}>
      <p>
        Word: <b>{puzzle.a}</b> · Decoy: <b>{puzzle.b}</b>
      </p>
      <p className="mt-2 text-sm">
        {result?.tied ? 'Vote tied; nobody was ejected.' : `Player ${(result?.ejected ?? 0) + 1} was ejected.`}
      </p>
      <p className="mt-2 text-sm">Imposter: Player {imposters[0]! + 1}</p>
      <p className="mt-1 text-xs">
        {imposters.length > 1 ? `All imposters: ${imposters.map(index => `Player ${index + 1}`).join(', ')}` : ''}
      </p>
      <Button
        className="mt-3"
        onClick={() => {
    ...
    () =>
      createSubmissionMachine(
        (input: { answers: number[]; elapsed_ms: number; exam: string }, idempotencyKey) =>
          playRpc('quizverse_words_duel_submit', { ...input, idempotency_key: idempotencyKey }) as Promise<
  ...
  const load = async () => {
    ...
    try {
      ...
      if (response.alreadySubmitted) {
        setMessage(
          `Today’s ${exam.toUpperCase()} duel is already submitted · score ${response.previousScore ?? 'recorded'}/10.`
        )
        setPhase('result')
      ...
      if (response.questions.length !== 10) {
        throw new Error('The authoritative duel did not return ten valid questions.')
      }
  ...
  return (
    <GameCard title="Vocab Duel">
      {phase === 'select' && (
        <>
          <div className="flex gap-2">
            {['gre', 'gmat', 'ielts'].map(value => (
              <Button key={value} onClick={() => setExam(value)} variant={exam === value ? 'default' : 'outline'}>
                {value.toUpperCase()}
              </Button>
            ))}
          </div>
          <div className="mt-3 flex gap-2">
            <Button onClick={() => void load()}>Start today’s duel</Button>
            <Button onClick={() => void loadLeaderboard()} variant="outline">
              Leaderboard
            </Button>
          </div>
        </>
      ...
      {phase === 'confirm' && (
        <>
          <p className="text-sm">
            All ten answers are ready. Submission affects today’s ranked duel and cannot be replayed.
          </p>
          <Button className="mt-3" onClick={() => void submit()}>
            Confirm ranked submission
          </Button>
        </>
      )}
      ...
      {phase === 'error' && (
        <>
          ...
          <Button onClick={() => setPhase('select')}>Try again</Button>
        </>
      )}
      {phase === 'result' && (
        <>
          <Status message={message} />
          <CopyResult text={`QuizVerse ${exam.toUpperCase()} Vocab Duel\n${message}`} />
        </>
      )}
...
export function WordsSurface({ route }: WordsRouteProps) {
  ...
  const wrap = (content: ReactNode) => (
    <div className="grid gap-3">
      <div aria-label="Words vocabulary skin" className="flex gap-2" role="group">
        <Button onClick={() => setSkin('general')} variant={skin === 'general' ? 'default' : 'outline'}>
          General
        </Button>
        <Button onClick={() => setSkin('gre-easy')} variant={skin === 'gre-easy' ? 'default' : 'outline'}>
          GRE Easy
        </Button>
      </div>
      {content}
  ...
  if (route === 'daily') {
    return wrap(
      <WordsContentLoader key={`${day}:${skin}`} mode="daily" skin={skin}>
        {content => <DailyWord content={content} />}
      </WordsContentLoader>
    )
  }
  ...
  if (route === 'groups') {
    return wrap(
      <WordsContentLoader key={`${day}:${skin}`} mode="groups" skin={skin}>
        {content => <GroupsGame content={content} />}
      </WordsContentLoader>
    )
  }
  ...
  if (route === 'spell') {
    return wrap(
      <WordsContentLoader key={`${day}:${skin}`} mode="spell" skin={skin}>
        {content => <SpellGame content={content} />}
      </WordsContentLoader>
    )
  }
  ...
  if (route === 'crossword') {
    return wrap(
      <WordsContentLoader key={`${day}:${skin}`} mode="crossword" skin={skin}>
        {content => <CrosswordGame content={content} />}
      </WordsContentLoader>
    )
  }
  ...
  if (route === 'imposter') {
    return wrap(
      <WordsContentLoader key={`${day}:${skin}`} mode="imposter" skin={skin}>
        {content => <ImposterGame content={content} />}
      </WordsContentLoader>
    )
  }

  if (route === 'duel') {
    return wrap(<DuelGame key={`${day}:${skin}`} skin={skin} />)
  }

  return (
    <GameCard title="Words rules and provenance">
      <p className="text-sm">
        Daily Word uses six attempts and duplicate-aware letter scoring. Groups allows four mistakes. Spell requires the
        center letter and source-list validation. All daily content is selected by the source-compatible UTC FNV-1a
        seed, with explicit server or offline provenance.
      </p>
    </GameCard>
  )
}
...
function Status({ error = false, message }: { error?: boolean; message: string }) {
  return message ? (
    <p
      className={cn('mt-3 rounded p-2 text-xs', error ? 'bg-red-950 text-red-100' : 'bg-black/20')}
