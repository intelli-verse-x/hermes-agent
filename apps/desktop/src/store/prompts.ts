...
function queuedPromptStore<T extends KeyedPrompt>(): PromptStore<T> {
  ...
  // PromptStore consumers ($all readers like sessionApprovalRequest /
  // sessionAwaitingInput) expect one prompt per session — surface each
  // session's queue head, which is exactly the prompt being shown.
  const $heads = computed($all, all =>
    Object.fromEntries(
      Object.entries(all)
        .filter(([, queue]) => queue.length > 0)
        .map(([key, queue]) => [key, queue[0]])
    )
  )
