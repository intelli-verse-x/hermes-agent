...
const ApprovalBar: FC<{ request: ApprovalRequest; surface: 'floating' | 'inline' }> = ({ request, surface }) => {
  ...
  // Adaptive-cloud handoff approvals are one-shot by design (fork governance):
  // no session/permanent unlocks are offered for them.
  const hasMoreOptions = (allowSession || allowAlways) && request.approvalKind !== 'adaptive-cloud'
