...
test('canonicalizes exact QuizVerse product method and path templates', () => {
  assert.deepEqual(
    canonicalizeQuizverseProductRequest('/notes/note-1/chat?stream=1', 'POST'),
    {
      method: 'POST',
  ...
  assert.equal(
    canonicalizeQuizverseProductRequest('/flashcards/srs/queue', 'GET').requestPath,
    '/flashcards/srs/queue'
  )
  assert.equal(
  ...
  assert.deepEqual(
    canonicalizeQuizverseProductRequest('/notes/note-1', 'PUT'),
    {
      method: 'PUT',
  ...
  assert.deepEqual(
    canonicalizeQuizverseProductRequest('/api/words/daily?mode=spell&skin=gre-easy', 'GET'),
    {
      method: 'GET',
  ...
  assert.deepEqual(
    canonicalizeQuizverseProductRequest('/api/words/content/manifest', 'GET'),
    {
      method: 'GET',
  ...
  assert.deepEqual(
    canonicalizeQuizverseProductRequest('/api/lap/notes/create', 'POST'),
    {
      method: 'POST',
...
test('confirms only destructive, spend, and external-write product actions', () => {
  ...
  assert.equal(canonicalizeQuizverseProductRequest('/notes/note-1/generate-figurine', 'POST').requiresConfirmation, false)
...
test('enforces manifest and immutable dataset cache expiry', () => {
  ...
  assert.equal(isQuizverseWordsCacheFresh({
    body: '{}',
    contentType: 'application/json',
    etag: '"v1"',
    expiresAt: manifestExpiry,
    savedAt
  }, manifestExpiry - 1), true)
  assert.equal(isQuizverseWordsCacheFresh({
    body: '{}',
    contentType: 'application/json',
    etag: '"v1"',
...
test('rejects traversal, encoded bypasses, external URLs, and method mismatches', () => {
  ...
  assert.throws(
    () => canonicalizeQuizverseProductRequest('/flashcards/srs/queue', 'DELETE'),
    /method and path/
  )
  assert.throws(
    () => canonicalizeQuizverseProductRequest('/unknown', 'GET'),
    /method and path/
  )
})
