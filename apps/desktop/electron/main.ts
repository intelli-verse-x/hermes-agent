...
function resolveHermesHome() {
  ...
  if (process.env.HERMES_DESKTOP_BRAND === 'quizverse' || process.env.HERMES_DESKTOP_BRAND === 'foundrly') {
...
function ensureDesktopBrandProvision(targetHermesHome = effectiveDesktopHermesHome()) {
  ...
  if (process.env.HERMES_DESKTOP_BRAND === 'foundrly') {
    try {
      ...
      rememberLog(`[foundrly-mcp] provision failed: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
}
    ...
    app.on('web-contents-created', (_event, contents) => {
      ...
      contents.setWindowOpenHandler(({ url }) => {
        ...
        try {
          ...
          const stayInGuest = host === 'admin.intelli-verse-x.ai' || host.endsWith('.intelli-verse-x.ai')
