...
const { assertQuizverseIsolatedHome, resolveQuizverseEffectiveHermesHome } = await import(
  new URL('./qv-mcp-profile.ts', import.meta.url).href
)
...
test('resolves default and named QuizVerse profiles under the isolated root', () => {
  ...
  assert.equal(resolveQuizverseEffectiveHermesHome(base, 'student'), path.join(base, 'profiles', 'student'))
})
