"""
Configuration management for IX Agency.
...
#   the next ``subprocess.run([...])`` IX Agency makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. IX Agency itself starts
...
#   ``HERMES_ENV`` — IX Agency runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # IX Agency runtime location — never via dashboard env writer.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or IX Agency runtime location "
...
def is_managed() -> bool:
    """Check if IX Agency is running in package-manager-managed mode.
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how IX Agency was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* IX Agency lives in a ``uv tool`` layout.
    ...
    the *active* IX Agency is a regular pip/venv install, causing
...
#     — that script installs a *new* host-side IX Agency, it doesn't update
...
IX Agency runs as a published image (nousresearch/hermes-agent), not a
...
def format_managed_message(action: str = "modify this IX Agency installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this IX Agency installation is managed by NixOS "
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this IX Agency installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
    ...
    return (
        f"Cannot {action}: this IX Agency installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall IX Agency."
    )
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running IX Agency commonly set these to map the in-container
...
def _is_container() -> bool:
    ...
    When IX Agency runs in a container with volume-mounted config files, forcing
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the IX Agency-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps IX Agency
        ...
        # app, ACP) in a code workspace, IX Agency adds a coding operating brief
    ...
    "terminal": {
        ...
        # skipped. When empty, IX Agency auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), IX Agency sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when IX Agency
        ...
        # bundled IX Agency image, which drops to the `hermes` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, IX Agency sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and IX Agency should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # AGENTS.md, CLAUDE.md, .hermes.md, or .cursorrules before IX Agency applies
    ...
    # single read_file page exceeds these limits, IX Agency truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so IX Agency' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); hermes =
                                      # IX Agency' compression threshold triggers
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; IX Agency appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
    ...
    # response. If not, IX Agency feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before IX Agency auto-pauses the goal and
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *IX Agency*" header.
    ...
    # replace IX Agency' built-in platform hint for a single messaging platform
    ...
    "security": {
        ...
        "acked_advisories": [],
        # Allow IX Agency to lazy-install opt-in backend packages from PyPI
    ...
    #
    # Core IX Agency tools (terminal, read_file, write_file, patch,
    ...
    "gateway": {
        ...
        # behavior: files must live under the IX Agency cache, under
        ...
        # uploaded as native gateway attachments. Files inside the IX Agency
    ...
    "computer_use": {
        ...
        # by default upstream. IX Agency disables it for our users unless they
        # explicitly opt in here. When false (default), IX Agency sets
    ...
    # IX Agency Desktop (Electron app) launch options. These only affect
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials IX Agency mints short-lived tokens from. Falls back to "
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the IX Agency auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use IX Agency, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote IX Agency API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote IX Agency API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote IX Agency API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but IX Agency' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older IX Agency versions) may write
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — IX Agency won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset IX Agency writes itself."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real IX Agency env var names, avoiding false positives from values
...
def reload_env() -> int:
    ...
    the .env file (but only vars known to IX Agency — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known IX Agency vars that are no longer in .env
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    ...
    Used for IX Agency-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ IX Agency Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
